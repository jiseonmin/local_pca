...
usage <- "\
\
After running templated lostruct script (run_lostruct.R), compute the distance by chromosome.
Using the same options as run_lostruct.R
...
for (k in seq_along(bcf.files)) {
	...
	mds.file <- file.path( opt$outdir, paste0(chrom.name, ".mds_coords.csv") )
	cat("Found PCs for chromosome", chrom.name, ", now computing distance", "\n")
	# Distance matrix of the chromosome
	system.time(pc.distmat <- pc_dist( pca.stuff, npc=opt$npc ) )
	cat("Done computing distances for chromosome", chrom.name, ", now running MDS and writing results to", mds.file, "\n")
	
	na.inds <- is.na( pca.stuff[,1] ) # there may be windows with missing data
	mds.coords <- cbind( data.frame(
					chrom=rep(chrom.name, nrow(pca.stuff)),
					window=seq_len(nrow(pca.stuff))),
